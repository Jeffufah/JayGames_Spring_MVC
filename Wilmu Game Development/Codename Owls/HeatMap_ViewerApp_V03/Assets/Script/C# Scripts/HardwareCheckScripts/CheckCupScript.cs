...
public class CheckCupScript : MonoBehaviour
{
    ...
    public IEnumerator CheckCupStatus()
    {
        Debug.Log("Checking Cup status...");

        user = "schoolt5_owl";
        password = "hoot";

        sql1 = "Select `CupUsed` From `CupCheck`";
    ...
    public IEnumerator SetCupStatus()
    {
        Debug.Log("Setting Cup status...");

        user = "schoolt5_owl";
        password = "hoot";

        sql2 = "UPDATE  `CupCheck` SET  `CupUsed` =  '0' WHERE  `ID` =  '1'";
    ...
    IEnumerator Connect2(WWW w)
    {
        ...
        if (w.error == null)
        {
            ...
            if (w.text != "Connection Failed.Access denied for user '" + user + "'@'localhost' (using password: YES)")
            {
                ...
                //CheckCupStatus();
                gameObject.GetComponent<SpawnEffect>().SpawnCupIndicator();
                yield break;
            }
