...
public class PlayingSlot 
{
    ...
    private PlotFourGame plotFourGame;
     ...
     */
    PlayingSlot(PlotFourGame plotFourGame, double columnOffset, double rowOffset, ArrowIndicator[] arrowIndicators, PlayingSlot[][] playingGrid, ResetButton resetButton)
    {
        ...
        chip.setOnMouseClicked(clickEvent -> 
        {
            if (!GameManager.getIsGameOver())
            {
                if (clickEvent.getButton() == MouseButton.PRIMARY)
                {
                    if (chip.getFill() == Color.WHITE)
                    {
                        this.plotFourGame.dropChip(chip.getId(), playingGrid, arrowIndicators, resetButton);
                    }
                }
            }
        });
